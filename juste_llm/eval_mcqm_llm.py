...
# Importation du client depuis le générateur simple (sans RAG)
from app_llm import client

# Définition locale du device (car absent de app_llm.py)
device = "cuda" if torch.cuda.is_available() else "cpu"

# --- CONFIGURATION ---

TEST_FILE = "dataset_2/mcqm_test.json" 
TRAIN_FILE = "dataset_1/mcqm_train.json"  
LIMIT = 200                               
...
def executer_evaluation_qcm():
    ...
    print(f"🚀 Évaluation stricte du générateur LLM (SANS RAG) sur {LIMIT} questions de test (GPU accéléré)...")
    ...
    for _ in [1]:  # Astuce pour maintenir l'alignement de tes 8 espaces
        for line in toutes_les_lignes:  # Maintient l'alignement de tes 12 espaces
            ...
            # 1. Étape du Retriever (Désactivée car utilisation de app_llm)
            # Le contexte global est fixé arbitrairement pour ne pas briser la fonction NLI plus bas.
            contexte_global = "Génération directe basée sur les poids du modèle. Aucun contexte RAG fourni."

            # 2. Prompt ÉVALUATION Strict
            # Légèrement adapté pour un générateur sans RAG
            prompt_eval = f"""Tu es un agent d'évaluation de QCM médicaux. 
Analyse le cas, la question et les choix fournis ci-dessous.
Utilise l'intégralité de tes connaissances médicales pour identifier la ou les bonnes réponses.
...
"""
            ...
            # Attention: Sans RAG, ce score NLI reflètera l'entailment entre un contexte vide et la réponse. 
            # Il sera probablement très bas, ce qui est normal ici.
    ...
    print("\n" + "="*50)
    print("📊 RAPPORT FINAL DE PERFORMANCES (SANS RAG)")
    print("="*50)
