...
"""
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Tu dois répondre STRICTEMENT sous le format : RÉPONSE_FINALE : X, Y (ou juste X s'il n'y en a qu'une)."
                        )
                    },
                    {"role": "user", "content": prompt_eval}
                ],
                temperature=0.0 # On garde à 0 pour éviter qu'il hallucine
            )
            ...
            # --- NOUVELLE LOGIQUE : LA VRAIE RÉPONSE EST-ELLE DANS LES CHOIX DE L'IA ? ---
            # .issubset() vérifie si la (ou les) vraie(s) lettre(s) se trouve(nt) bien 
            # à l'intérieur des lettres données par l'IA.
            est_correct = vraies_lettres.issubset(lettres_predites) and len(vraies_lettres) > 0
